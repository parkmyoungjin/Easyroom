...
describe('AuthMigrationScript', () => {
  ...
  describe('Legacy State Detection', () => {
    ...
    it('detects legacy user-only state', async () => {
      const legacyUser = { id: 'user456', email: 'user@example.com' };
      
      localStorageMock.getItem.mockImplementation((key) => {
        if (key === 'easyroom_migration_log') return null;
        if (key === 'easyroom_user') return JSON.stringify(legacyUser);
        return null;
      });
  ...
  describe('Error Handling', () => {
    it('handles localStorage access errors during detection', async () => {
      ...
      expect(result.error).toContain('localStorage access denied');
    });
  ...
  describe('Logging', () => {
    ...
    it('logs cleanup operations', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('[Migration] Cleaned up legacy key: easyroom_auth')
      );
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('[Migration] Cleaned up legacy key: easyroom_token')
      );
    });
  });
});