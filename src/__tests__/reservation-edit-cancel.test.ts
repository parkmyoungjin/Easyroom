...
describe('사용자 ID 매핑 디버깅', () => {
  it('정상적인 ID 매핑을 감지해야 함', () => {
    const result = debugUserIdMapping(mockUserProfile, mockReservation);
    
    expect(result.mapping.profileIdMatchesAuthId).toBe(true);
    expect(result.mapping.reservationUserIdMatchesProfileId).toBe(false); // dbId와 매칭되어야 함
    expect(result.issues.length).toBeGreaterThan(0); // ID 불일치 이슈가 있어야 함
  });
  ...
  it('ID 매핑 문제를 감지해야 함', () => {
    const problematicProfile = {
      ...
      dbId: undefined, // dbId가 없는 경우
    };
    
    const result = debugUserIdMapping(problematicProfile, mockReservation);
    
    expect(result.mapping.profileIdMatchesAuthId).toBe(false);
