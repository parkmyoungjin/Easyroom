...
import { getClientStatus, isClientReady } from '@/lib/supabase/client';
...
class EnvironmentHealthChecker {
  ...
  private async checkSupabaseClient(deepCheck: boolean): Promise<ComponentHealth> {
    try {
      ...
      if (isReady && clientStatus.state === 'ready') {
        return {
          status: 'healthy',
          ...
          metrics: {
            ...
            state: clientStatus.state === 'ready' ? 1 : 0
          }
        };
      }

      if (clientStatus.state === 'initializing') {
        return {
          status: 'degraded',
          message: 'Supabase client is initializing',
          lastCheck: new Date(),
          metrics: {
            retryCount: clientStatus.retryCount,
            state: 0
      ...
      if (clientStatus.state === 'retrying') {
        return {
          status: 'degraded',
          message: `Supabase client is retrying (attempt ${clientStatus.retryCount})`,
          lastCheck: new Date(),
          metrics: {
            retryCount: clientStatus.retryCount,
            state: 0
      ...
      return {
        status: 'critical',
        message: `Supabase client is in error state: ${clientStatus.state}`,
        lastCheck: new Date(),
        metrics: {
          retryCount: clientStatus.retryCount,
          state: 0
        },
        errors: clientStatus.lastError ? [clientStatus.lastError.message] : undefined
      };
