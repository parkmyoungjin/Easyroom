...
describe('NavigationController', () => {
  ...
  describe('getRedirectPath', () => {
    ...
    it('should return dashboard for employee users', () => {
    ...
    it('should not redirect to auth pages', () => {
      ...
      expect(result).toBe('/dashboard'); // Falls back to role-based default
    });
  ...
  describe('handlePostLoginRedirect', () => {
    ...
    it('should handle redirect timeout', async () => {
      ...
      expect(mockConsoleWarn).toHaveBeenCalledWith(
        expect.stringContaining('Redirect timeout')
      );
    });

    it('should use fallback path on timeout', () => {
      // Directly test the timeout handler
      const originalHandleRedirectTimeout = (navigationController as any).handleRedirectTimeout;
      (navigationController as any).handleRedirectTimeout = jest.fn();

      navigationController.handlePostLoginRedirect({
        userProfile: mockUserProfile,
      ...
      expect((navigationController as any).handleRedirectTimeout).toHaveBeenCalledWith(
        '/dashboard',
        '/fallback'
      );

      // Restore original method
      (navigationController as any).handleRedirectTimeout = originalHandleRedirectTimeout;
    });
  });
