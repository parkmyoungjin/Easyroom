...
class EnhancedEmailValidationService implements EmailValidationService {
  ...
  private async attemptEmailCheckWithRetry(email: string, retryCount: number): Promise<EmailCheckResult> {
    try {
      ...
      const { createClient, getClientStatus, initializeClient } = await import('@/lib/supabase/client');
      
      // Check client status first
      const clientStatus = getClientStatus();
      
      if (clientStatus.state === 'error' || clientStatus.state === 'uninitialized') {
        // Try to initialize client
        const initResult = await initializeClient();
        if (!initResult.success) {
          return this.handleClientInitializationError(initResult.error!, email, retryCount);
        }
      }
