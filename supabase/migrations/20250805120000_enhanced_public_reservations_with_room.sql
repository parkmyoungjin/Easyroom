-- Enhanced public reservations function with room information
-- 살아있는 현황판을 위한 room_name 포함 함수
...
-- 권한 부여
GRANT EXECUTE ON FUNCTION public.get_public_reservations_with_room(timestamptz, timestamptz, integer, integer) TO anon;
GRANT EXECUTE ON FUNCTION public.get_public_reservations_with_room(timestamptz, timestamptz, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_public_reservations_with_room(timestamptz, timestamptz, integer, integer) TO service_role;

-- 기존 get_reservations_for_period 함수도 room 정보 포함하도록 업데이트
